...
(s=$(cat /etc/system-release); if [[ -z $s ]]; then s=$(cat /etc/redhat-release); fi; if [[ -z $s ]]; then s=$(cat /etc/os-release | grep PRETTY | cut -d '=' -f2 | sed -e 's/^\"//' -e 's/\"$//'); fi; s=$(echo ${s} | tr '[:upper:]' '[:lower:]'); flag=1; if [[ -n $(echo $s | grep 'centos' ) ]] || [[ -n $(echo $s | grep 'red') && -n $(echo $s | grep 'hat') ]]; then IFS=$'\n'; a=($(rpm -q kernel --last | awk -F '[[:space:]][[:space:]]+' '{printf "%s,%s\n",$1,$2}')); elif [[ -n $(echo $s | grep 'buntu' ) ]]; then IFS=$'\n'; a=($(dpkg --get-selections | grep linux-image | awk '{print $1 s n}' s=',' n='NA' )); else flag=0; fi; if [[ ${flag} -eq 0 ]]; then echo \\\"HotFixID\\\":\\\"\\\",\\\"InstalledOn\\\":\\\"\\\"; else val=\"\"; for ((i = 0; i < ${#a[@]}; i++)); do p=\\\"HotFixID\\\":\\\"$(echo ${a[i]} | cut -d , -f1)\\\",\\\"InstalledOn\\\":\\\"$(if [ $(locale | grep 'LANG' | grep 'zh_') ]; then echo ${a[i]} | cut -d , -f2 | grep -Eo '[0-9]+' | tr '\\n' ' ' | awk '{print $1$2$3}' | { read gmt ; date +%m/%d/%Y -d ${gmt} ; }; else echo ${a[i]} | cut -d , -f2 | { read gmt ; date +%m/%d/%Y -d ${gmt} ; }; fi;)\\\"; val=${val}${p}~; done; fi; echo ${val:0:${#val}-1};)
